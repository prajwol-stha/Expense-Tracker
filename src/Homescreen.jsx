...
const Homescreen = () => {
  ...
  const saveExpense1 = async () => {
    if (!isConnected) {
      ...
      Alert.alert(
        ...
        'Please connect to the internet to save expense.',
    ...
    if (spentAmount.trim() === '' || remarks.trim() === '') {
      Alert.alert(
        ...
        'Please enter both spent amount and remarks.',
        [{text: 'OK', onPress: () => console.log('OK Pressed')}],
      );
      return;
    }
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        Alert.alert(
          'Database Updated',
          `Added Nrs ${parseFloat(
            spentAmount,
          )} to database on ${formattedDate}.`,
          [
            {
              text: 'OK',
              onPress: () => console.log('Database update successful'),
            },
          ],
        );
        ...
        console.error(
          'Error saving expense. Unexpected status code:',
          response.status,
        );
      }
      setSpentAmount('');
  ...
  const saveExpenseFromLocal = async (spentAmount, remarks, date) => {
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        Alert.alert(
          'Database Updated',
          `Added Nrs ${parseFloat(spentAmount)} to database on ${date}.`,
          [{text: 'OK', onPress: () => console.log('OK Pressed')}],
        );
        console.log('Expense saved successfully:', response.data);
      } else {
      ...
      if (allKeys.length > 0) {
        ...
        allData.forEach(([key, value]) => {
          try {
            ...
            console.error(
              `Error parsing data for key ${key}:`,
              parseError.message,
            );
          }
        });
  ...
  const handleStatement = () => {
    if (!isConnected) {
      Alert.alert(
        'No Internet Connection',
        'Please connect to the internet to view the statement.',
        [{text: 'OK', onPress: () => console.log('OK Pressed')}],
      );
      return;
    }
  ...
  const handleSaveToLocalStorage = async () => {
    ...
    try {
      ...
      console.error(
        'Error saving expense data to local storage:',
        error.message,
      );
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {isConnected && (
        <>
          <TouchableOpacity
            onPress={saveExpense1}
            style={[styles.btn, !isConnected]}>
      ...
      {!isConnected && (
        <>
          <TouchableOpacity
            ...
            style={[styles.btn, styles.btnOffline]}>
            <Text style={styles.btnText}>Save to local storage</Text>
          </TouchableOpacity>

          <TouchableOpacity
            onPress={handleGetLocalData}
            style={[styles.btn, styles.btnOffline]}>
